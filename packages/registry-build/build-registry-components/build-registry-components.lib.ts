...
import { ScriptKind, type SourceFile } from 'ts-morph'
import { ENV } from '../main/main.constants'
import { create_temp_source_file, project } from '../main/main.lib'
import type {
...
export async function get_file_content({ file, spinner }: GetFileContentParams): Promise<string | undefined> {
  try {
    const filePath = path.join(
      process.cwd(),
      `../${
        file.type.includes('ui')
          ? ENV.REGISTRY_UI_PATH
          : file.type.includes('example')
            ? ENV.REGISTRY_EXAMPLES_PATH
            : file.type.includes('internal')
