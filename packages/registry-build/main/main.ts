...
export async function main() {
  ...
  for (const item of index) {
    ...
    // 2- build the __registry__/ entry with dynamic import.
    // UI entries are intentionally excluded from __ui_registry__/index.tsx.
    if (item.type === 'registry:ui' || item.type === 'registry:lib' || item.type === 'registry:hook') {
      continue
    }
