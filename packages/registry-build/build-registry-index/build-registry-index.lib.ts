...
import { ENV } from '../main/main.constants'
import type { GetComponentFilesArgs } from './build-registry-index.types'
...
export async function get_component_files({
  ...
  try {
    ...
    return {
      ...
      source: `${type.includes('ui') ? ENV.REGISTRY_UI_PATH : type.includes('example') ? ENV.REGISTRY_EXAMPLES_PATH : type.includes('internal') ? ENV.REGISTRY_INTERNAL_PATH : ENV.REGISTRY_BLOCKS_PATH}${item.root_folder}`,
      tree,
    }
  } catch (error) {
