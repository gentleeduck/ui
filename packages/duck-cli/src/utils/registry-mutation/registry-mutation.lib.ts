...
export async function get_installation_config(
  ...
  try {
    ...
    // try to find the alias in the paths
    let write_path_key = Object.keys(ts_config.compilerOptions.paths).find(
      (path) => path.includes(alias)
    );

    let alias_was_swapped = false;
    // if not found try to swap ~ with @ or viceversa
    if (!write_path_key) {
      const swapped = alias.startsWith("~") ? "@" : "~";
      write_path_key = Object.keys(ts_config.compilerOptions.paths).find(
        (path) => path.includes(swapped)
      );

      if (write_path_key) {
        alias_was_swapped = true;
      }
    }
...
Make sure your ${highlighter.info(
          "duck-ui.config.json"
        )} and ${highlighter.info("tsconfig.json")} aliases match.`
      );
      process.exit(1);
    }

    // Warn user if alias mismatch was auto-corrected
    if (alias_was_swapped) {
      spinner.warn(
        `Alias mismatch detected.
Using "${write_path_key}" instead of "${alias}/*". 
To avoid confusion, update your configs so both use the same alias.`
      );
    }
    ...
    if (!options.yes) {
      ...
      const { yes } = await prompts({
        initial: options.yes,
        message: `💡 Do you want to install ${highlighter.info(
          "components"
        )}? at ${highlighter.warn(write_path)}`,
        name: "yes",
        type: "confirm",
      });
      spinner.start();
...
export async function process_components(
  ...
  try {
    ...
    await install_registry_dependencies(
      dependencies,
      spinner,
      write_path,
      options.force
...
async function install_component(
  ...
  spinner.text = `🦆 Installing ${
    registry ? "necessary " : ""
  }component: ${highlighter.info(`${component.name}`)}`;
...
export async function install_registry_dependencies(
  ...
  async function fetchAndProcess(deps: Set<string>) {
    ...
    spinner.succeed(
      `🦋 Fetched ${components.length} necessary component${
        components.length > 1 ? "s" : ""
      } from registry`
    );
  ...
  const initialDeps = new Set(
    dependencies.registry_dependencies.map((d) => d.toLowerCase())
  );
  ...
  dependencies.dependencies = Array.from(
    new Set(dependencies.dependencies ?? [])
  );
  dependencies.dev_dependencies = Array.from(
    new Set(dependencies.dev_dependencies ?? [])
  );
...
export async function process_component_files(
  ...
  const file_path = path.resolve(
    `${write_path}/${component_type}`,
    component.root_folder
  );
  if (!force) {
    if (fs.existsSync(file_path) && fs.readdirSync(file_path).length > 0) {
      ...
      const { overwrite } = await prompts({
        initial: true,
        message: `💡 Do you want to overwrite ${highlighter.info(
          component.name
        )}?`,
        name: "overwrite",
        type: "confirm",
      ...
      if (!overwrite) {
        spinner.warn(
          `🦆 Components already exists: ${highlighter.info(
            `${component_type}${component.root_folder}`
          )} (skipping)`
        );
        return;
      }
...
export async function process_component_dependencies(
  ...
  try {
    ...
    spinner.text = `🔧 Installing ${highlighter.info(
