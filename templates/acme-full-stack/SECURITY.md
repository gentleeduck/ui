...
Pre-1.0. Only the latest minor of each `@acme/*` package is
...
- the affected package and version
  (e.g. `bun pm ls @acme/registry-ui`)
...
`@acme/ui` ships React components, primitives, and a CLI that
runs in dev/build environments. Relevant attack surfaces:

- **CLI (`@acme/cli`)**: writes files, runs package-manager
...
- **Registry source (`@acme/registry-ui`)**: components are
...
Thank you for helping keep acme/ui secure.
