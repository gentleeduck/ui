...
The `toFieldErrors` helper normalizes TanStack Form's error format (which can be strings or objects) into the `{ message: string }` format expected by the `<FieldError />` component.
