---
title: types & namespaces
description: I-prefixed interfaces under per-module namespaces - the type-import contract for duck-iam consumers.
section: Overview
---
## Convention
...
function check(policy: AccessControl.IPolicy, req: Request.IAccessRequest): AccessControl.IDecision {
  // ...
}
```
...
| `EngineTypes` | `IConfig`, `IHooks`, `IAdmin`, `IMetricsEvent`, `IInvalidator`, `IInvalidateEvent`, `ISnapshot`, `IImportOptions`, `IImportResult`, `IHealth` |
...
The class `Engine` lives next to the `EngineTypes` namespace - same name avoided by suffixing the type-only namespace. `MemoryAdapter` / `FileAdapter` / etc. are classes; their config namespaces (`Memory`, `File`, `Http`, `Redis`, `Drizzle`) hold the option bags. `ReactClient` is named so to avoid clashing with the React package namespace at the import site.
...
**1. Avoid name collisions across modules.** `IResult` exists in both `Validate` and `Explain`. Without namespaces, one of them needs a longer name. With namespaces, both stay short and the context is explicit at the call site (`Validate.IResult` vs `Explain.IResult`).
...
The runtime values (`Engine`, `MemoryAdapter`, `policy`, `defineRole`, builders, etc.) are unchanged - only type imports moved.

---

## See also

- [Engine](/duck-iam/advanced/engine) - `EngineTypes.IConfig` and `EngineTypes.IHooks` shapes.
- [Adapters](/duck-iam/integrations/adapters) - `Adapter.IAdapter` contract.
