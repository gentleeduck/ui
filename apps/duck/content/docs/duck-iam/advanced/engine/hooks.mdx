...
```typescript
import type { EngineTypes } from '@gentleduck/iam'
...
All six hooks live under `EngineTypes.IHooks`. `afterEvaluate` and `onDeny` only fire in development mode (they receive the full `AccessControl.IDecision`); `onMetrics` and `onError` fire in both modes.
...
Lightweight telemetry hook called once per evaluation. Receives a primitive-only `EngineTypes.IMetricsEvent` - no `IDecision` object, no allocation cost beyond the event itself. Wire it for latency, hit-rate, and outcome metrics in both development and production modes.
...
Event shape (`EngineTypes.IMetricsEvent`):
