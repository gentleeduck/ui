...
The constructor accepts an `EngineTypes.IConfig`:

```typescript
interface IConfig {
  adapter: Adapter.IAdapter             // Required. Storage backend.
  ...
  hooks?: EngineTypes.IHooks            // Lifecycle hooks.
  ...
  invalidator?: EngineTypes.IInvalidator // Cross-instance cache invalidation broadcaster.
...
| --- | --- | --- | --- |
| `adapter` | `Adapter.IAdapter` | -- | The storage backend. Required. |
...
| `hooks` | `EngineTypes.IHooks` | `{}` | Lifecycle hooks - see [hooks](/duck-iam/advanced/engine/hooks) |
...
| `invalidator` | `EngineTypes.IInvalidator` | -- | Cross-instance pub/sub broadcaster - see [caching](/duck-iam/advanced/engine/caching) |
...
`EngineTypes.IHooks` accepts the lifecycle hooks plus two operational hooks:
