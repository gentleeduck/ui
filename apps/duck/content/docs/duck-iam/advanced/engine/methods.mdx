...
engine.can(
  ...
  environment?: Request.IAccessRequest['environment'],
  scope?: TScope,
): Promise<boolean>
```
...
Takes a complete `Request.IAccessRequest` object. The low-level method that `can()` and `check()` call internally. Use it when you already have a resolved subject:
...
Liveness + readiness probe. One timed-out `listPolicies` round-trip plus a cache-hit-rate snapshot. Wire to `/healthz`. Returns an `EngineTypes.IHealth`:
...
engine.resetStats() // zero the counters
```
...
| Pre-built `Request.IAccessRequest` | `authorize()` |
