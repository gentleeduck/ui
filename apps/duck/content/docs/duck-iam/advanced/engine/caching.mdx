...
Want NATS / Kafka / AMQP / your in-process bus? Implement the `EngineTypes.IInvalidator` contract:
...
Delivery is at-least-once: `engine.invalidate*()` methods are idempotent so re-applying the same event is safe.
