...
Resolves a dot-path string against a `Request.IAccessRequest`. The engine uses this to extract
...
```typescript
function resolve(request: Request.IAccessRequest, path: string): AttributeValue
```
...
- **HttpAdapter null on 404.** `HttpAdapter.getPolicy(id)` and `HttpAdapter.getRole(id)`
...
class MyAdapter implements Adapter.IAdapter {
  async listPolicies() {
...
`parseRoleRow` mirrors `parsePolicyRow` for `Adapter.IRoleStore.listRoles`
