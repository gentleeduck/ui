...
All validators return a `Validate.IResult` with `valid: boolean` and
an array of `Validate.IIssue` records. Issues have closed-set `code`
...
The full `Validate.IIssue` type is exported from
