...
`createMetricsAggregator` is a ring-buffer histogram over `EngineTypes.IMetricsEvent`. Wire its `.record` method to `hooks.onMetrics` and call `.snapshot()` on demand - a `/metrics` route, a Prometheus scrape, an OTel exporter, whatever your pipeline expects.
...
- `Metrics.ISnapshot` - shape returned by `.snapshot()` (`total`, `allow`, `deny`, `p50`, `p95`, `p99`, `max`, `samples`).
