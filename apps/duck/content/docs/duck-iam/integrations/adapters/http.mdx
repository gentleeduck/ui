...
`getPolicy(id)` and `getRole(id)` return `null` on a `404`, matching the `Adapter.IAdapter` contract.
...
HttpAdapter ships full SRE plumbing built-in. All four knobs are optional:
...
- `Http.IConfig` - constructor config for `HttpAdapter` (baseUrl, headers, fetch, retry / timeout / circuit breaker knobs).
