---
title: file adapter
description: Filesystem-backed JSON adapter for CLIs, dev fixtures, and single-process apps with modest policy counts.
...
```typescript
import type { File } from '@gentleduck/iam/adapters/file'

const init: File.IInit = {
  path: '/var/lib/myapp/iam.json',
...
| `fs`   | `File.IFS` | Filesystem driver - `node:fs/promises`-compatible. See "Custom FS" below. |
...
`File.IFS` is the minimal `node:fs/promises` surface the adapter needs. Implement it for in-memory testing, S3-backed durability, or any other backend:

```typescript
import type { File } from '@gentleduck/iam/adapters/file'

const inMemoryFS: File.IFS = {
...
const adapter = new FileAdapter({ path: '/store.json', fs: inMemoryFS })
```
...
```typescript
// File doesn't exist
const adapter = new FileAdapter({ path: '/nope/iam.json', fs })
await adapter.listPolicies() // -> []

// File is invalid JSON
fs.writeFileSync('/store.json', 'not-json{')
const adapter2 = new FileAdapter({ path: '/store.json', fs })
...
- [Custom adapter](/duck-iam/integrations/adapters/custom) - implement `Adapter.IAdapter` for any backend.
