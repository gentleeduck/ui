...
description: Route handler wrapper, server component checks, permission map generation, and edge middleware for Next.js.
...
<Callout type="warning" title="SEC-101 - `getUserId` is required in 2.1.0">
...
`withAccess` options conform to `Next.IWithAccessOptions`, `createNextMiddleware` options conform to `Next.IMiddlewareOptions`, and `createAdminHandlers` options conform to `Next.IAdminOptions` with `authorize` of shape `Next.IAdminAuthorize`.
...
- `Next.IWithAccessOptions` - options for `withAccess` (`getUserId`, `getEnvironment`, `scope`, `onError`).
- `Next.IMiddlewareOptions` - options for `createNextMiddleware` (`rules`, `getUserId`, `onError`).
- `Next.IAdminAuthorize` - signature of the required `authorize` callback for `createAdminHandlers`.
- `Next.IAdminOptions` - options for `createAdminHandlers`.

```typescript
import type { Next } from '@gentleduck/iam/server/next'

const opts: Next.IWithAccessOptions = {
  getUserId: async (req) => (await auth())?.user?.id ?? null,
}

const adminAuth: Next.IAdminAuthorize = async (req) => {
  const session = await getSession(req)
  return session?.user?.role === 'platform-admin'
}
```
