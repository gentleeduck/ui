...
**Core bundle headline +96% (~21 → 41 KB)** _if_ you import the everything-barrel. The 2.1.0 bundle-slim cycle dropped this from 44.8 → 41.3 KB by (1) dropping adapter re-exports from the barrel, (2) lazy-loading the validator chunk, (3) splitting `builder` / `explain` / `validate` into separate subpath entries. Realistic deployments measure 15-25 KB — see below.
