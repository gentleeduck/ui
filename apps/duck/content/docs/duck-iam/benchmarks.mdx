...
**duck-iam is larger than CASL.** ~45 KB vs ~6 KB. Bundle includes policy engine, RBAC-to-ABAC converter, explain tracer, builder, config validator, LRU cache, snapshot serializer, observability aggregator, plus the 2.1.0 safety wrappers and per-Engine cache primitives.

---

## 2.1.0 measured (before / after the security cycle)

Numbers from `scripts/benchmark.ts` (microsecond-per-op). Bundle sizes from `gzip -c | wc -c` on `dist/`. Reproduce via `bun run benchmark` in `packages/duck-iam/`. Machine-relative; run on your own hardware for absolute comparisons.

### Core paths

| Path | 2.0.0 (pre-cycle) | 2.1.0 (post-cycle) | Δ |
|---|---|---|---|
| `evaluatePolicy` (simple rule) | 0.68 µs | 0.78 µs | +15% |
| `evaluatePolicy` (conditions) | 1.33 µs | 1.00 µs | **−25%** |
| `evaluatePolicy` (target match) | 0.47 µs | 0.70 µs | +49% |
| `evaluatePolicy` (target skip) | 0.24 µs | 0.26 µs | +8% |
| `evaluate` (2 policies) | 0.65 µs | 1.16 µs | +78% |
| `evaluate` (deny path) | 0.58 µs | 0.73 µs | +26% |

### Engine paths (cache-warm)

| Path | 2.0.0 | 2.1.0 | Δ |
|---|---|---|---|
| `engine.can()` | 4.86 µs | 5.85 µs | +20% |
| `engine.check()` | 4.60 µs | 4.40 µs | **−4%** |
| `engine.permissions()` (20 checks) | 20.06 µs | 42.71 µs | **+113%** |
| `engine.explain()` | n/a | 7.40 µs | new |

### Bundle (gzipped, post-cycle)

| Module | Size |
|---|---|
| Core (full) | 44.8 KB |
| Core only (tree-shake target) | 0.7 KB |
...
Core was ~21 KB pre-cycle → 44.8 KB post-cycle (+113%). The diff is the safety wrapper set (`_safeHookCall`, sentinel-compare singleFlight helpers, per-request `IEvalSignals`, default CSRF predicate, `runAdminAuthz` / `withAdminAudit`, per-Engine cache primitives, Lua EVAL path for cross-process Redis migration). Adapters and clients stay tight (1–6 KB each).

### Honest take

**`engine.permissions()` doubled.** Each batch check now fires `onMetrics` + builds `IEvalSignals` + threads `onPolicyError` and the catch arm wraps via `_safeHookCall`. 4 extra allocations per check. The 2x cost bought silent-fail-open elimination in batch UI gates — previously a real bug, now structurally impossible.

**Core bundle +113%.** Each safety wrapper is small; the sum doubles ship size. Adapters and clients unchanged.

**`engine.check()` slightly faster.** Outer try-catch restructure lands in a tighter shape. Within noise.

**`evaluatePolicy(conditions)` faster.** Per-Engine cache threading de-bounces redundant resolution work in the hot path.
