...
| Path | 2.0.0 | 2.1.0 | Δ |
|---|---|---|---|
| `evaluatePolicy` (simple rule) | 0.68 µs | 0.76 µs | +12% |
| `evaluatePolicy` (conditions) | 1.33 µs | 1.00 µs | **−25%** |
| `evaluatePolicy` (target match) | 0.47 µs | 0.68 µs | +45% |
| `evaluatePolicy` (target skip) | 0.24 µs | 0.24 µs | 0% |
| `evaluate` (2 policies) | 0.65 µs | 1.24 µs | +91% |
| `evaluate` (deny path) | 0.58 µs | 0.62 µs | +7% |

### Engine paths (cache-warm)

| Path | 2.0.0 | 2.1.0 | Δ |
|---|---|---|---|
| `engine.can()` | 4.86 µs | 5.18 µs | +7% |
| `engine.check()` | 4.60 µs | 4.60 µs | 0% |
| `engine.permissions()` (20 checks) | 20.06 µs | 48.08 µs | **+140%** |
| `engine.explain()` | n/a | 8.02 µs | new |
...
**`engine.can()` ~7% slower.** 0.32 µs added per cache-warm check. You'd need 1 million checks per second sustained before this is measurable.
...
```ts
// ✅ Tight: import only what you use
import { Engine } from '@gentleduck/iam/core'
import { MemoryAdapter } from '@gentleduck/iam/adapters/memory'
import { adminRouter } from '@gentleduck/iam/server/express'

// ❌ Pulls everything (41 KB)
import { Engine, MemoryAdapter } from '@gentleduck/iam'
```
...
// ✅ Typical backend — pulls engine + adapter + server middleware only
import { Engine } from '@gentleduck/iam/core'
import { MemoryAdapter } from '@gentleduck/iam/adapters/memory'
import { adminRouter } from '@gentleduck/iam/server/express'

// ✅ Admin-write path — validator is lazy-loaded on first call
import { Engine } from '@gentleduck/iam/core'
await engine.admin.savePolicy(p) // pulls validate chunk on first call

// ❌ Pulls everything (41 KB) — avoid the barrel import
import { Engine, MemoryAdapter } from '@gentleduck/iam'
```
