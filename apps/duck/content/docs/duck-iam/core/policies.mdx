...
Dollar-prefixed values resolve at evaluation time instead of being literals. Use them
to compare two fields on the same request.
...
`$subject.id` isn't compared literally. At eval time the engine strips `$`, resolves
`subject.id` from the request, then compares.
...
The `isOwner()` shortcut uses this:
...
`$`-references work anywhere a value is accepted — `.check()`, `.attr()`,
`.resourceAttr()`, and `.env()`:
...
The `When` builder defaults to AND. Use `and()`, `or()`, and `not()` for nested
groups.
