---
title: primitives
description: Core data types - Subject, Resource, Action, Scope, Environment, AccessRequest, Decision. The shapes every check is built on.
section: Core
---
## The request side
...
```typescript
interface Request.ISubject {
  id: string
  roles: readonly string[]
  scopedRoles?: readonly Request.IScopedRole[]
...
```typescript
interface Request.IEnvironment {
  ip?: string
  userAgent?: string
  timestamp?: number
  [key: string]: Primitives.AttributeValue | undefined // custom fields
}
```

The server middleware integrations (Express, Hono, Nest, Next) build a default `Request.IEnvironment` from request headers. Add custom fields like `region`, `dayOfWeek`, `maintenanceMode`, etc. for time/geo/feature-flag rules.
...
```typescript
interface Request.IAccessRequest {
  subject: Request.ISubject
  action: string
  resource: Request.IResource
  scope?: string
  environment?: Request.IEnvironment
}
```

`engine.can()` and `engine.check()` build this object internally - you supply the parts. `engine.authorize()` accepts a pre-built `Request.IAccessRequest` directly for advanced use.
...
interface AccessControl.IRule {
  ...
  metadata?: Readonly<Primitives.Attributes>
}
```
