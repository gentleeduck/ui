...
description: How policies combine across an engine - AND (default), allow-overrides, first-applicable, NotApplicable semantics, and how intra-policy combining stays separate.
section: Core
---
...
2. **Across policies** - the engine merges per-policy decisions using its `policyCombine` setting (`AccessControl.PolicyCombine`). Defaults to `'and'`; configurable via `EngineTypes.IConfig.policyCombine`.
...
duck-iam now ships three cross-policy combine modes via `EngineTypes.IConfig.policyCombine`:
