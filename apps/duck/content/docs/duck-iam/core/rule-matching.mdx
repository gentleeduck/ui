...
For conditional rules, the engine evaluates the condition tree against the `Request.IAccessRequest`. Failed conditions cause the rule to be skipped, not the whole policy.

---

## Field resolution

Condition `field` values are dot-notation paths resolved against the `Request.IAccessRequest`.
