...
description: Every engine method, hook, cache, batch permission, explain/debug, and Admin API. Build a complete authorization backend.
section: Course
order: 4
---
## Goal
...
The low-level method takes a full `Request.IAccessRequest` object:

```typescript
import type { Request } from '@gentleduck/iam'

const request: Request.IAccessRequest = {
  subject: {
    id: 'bob',
    roles: ['editor', 'viewer'],
