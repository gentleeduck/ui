...
<MermaidDiagram chart={`flowchart TD
    ...
    subgraph Config["EngineTypes.IConfig"]
...
- [ ] Set `mode: 'production'` on high-traffic servers; use `createAccessConfig` with `as const`
- [ ] Replace `MemoryAdapter` with `PrismaAdapter`, `DrizzleAdapter`, or `HttpAdapter`
...
- [ ] Expose `engine.stats()` on an internal endpoint to watch cache hit rate, evictions, and adapter latency
- [ ] Use `engine.admin.export()` / `engine.admin.import()` to promote roles + policies between environments
- [ ] If using `HttpAdapter`, set `retries` and `circuitBreakerThreshold` to survive flaky upstreams
