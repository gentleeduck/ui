...
`@gentleduck/calendar` ships zero CSS. Instead, it outputs `data-*` attributes on every element that you use as styling hooks.
...
When using `@gentleduck/primitives/calendar`, each component adds a `data-slot` attribute. This lets you style components by their semantic role:
