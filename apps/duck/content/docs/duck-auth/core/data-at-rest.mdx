...
duck-auth ships two `DataAtRest.IAdapter` implementations so sensitive
...
rotation. Wire `dataAtRest` into your AuthRoot or via the bridge your
...
require a non-null `dataAtRest` on the AuthRoot. FIPS further requires
