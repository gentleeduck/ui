---
title: Events
description: InMemoryEvents, RedisEvents, the typed event taxonomy, and WebhookDeliverer for signed outbound delivery.
section: Core
---
...
`Events.IBus`. The default is `InMemoryEvents`; production deployments
swap to `RedisEvents` for cross-process fan-out.
...
`RedisEvents` publishes on a Pub/Sub channel and replays through the
