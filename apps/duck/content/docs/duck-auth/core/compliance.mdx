...
Presets return an `AuthRoot.IConfig` shape that you spread into
`new AuthRoot({ ... })`. Add your own keys after to override.
...
`AuthRoot.strict({ env: 'production' })` cross-checks the preset:
