---
title: Core overview
description: AuthRoot is the single faceted root. Sessions, identities, passwords, providers, MFA, flows, API keys, M2M, orgs, idempotency, hijack policy, anomaly — all on one instance.
section: Core
---

## What is `AuthRoot`?

`AuthRoot` is the runtime entry point for duck-auth. You instantiate it once
...
export const auth = new AuthRoot({ /* config */ })
```
...
| Facet | What it owns |
| --- | --- |
| `sessions` | Create / rotate / revoke / resolve sessions. The fixation-defence rotation matrix (DESIGN §37) routes every privilege change through `rotateOrCreate()`. |
| `identities` | CRUD + linking + merge + GDPR (soft-delete, restore, erase, exportAll, bulkCreate). Typed `Profile` generic. |
| `passwords` | Hash / verify / `needsRehash` against the configured `Hasher.IHasher`. Auto-rehash on successful sign-in if hasher params drift. |
...
| `limiter` | Rate-limiter. Use `MemoryLimiter` in dev; Redis-backed in production. `strict()` rejects `NoopLimiter` in production. |
...
| `providers` | Array of `Provider.IProvider` instances to register at boot. |
| `events` | `Events.IBus`. Default is `InMemoryEvents` — swap to `RedisEvents` or a custom bus for fan-out. |
...
`AuthRoot.strict()` is a boot-time validator. Call it after construction
...
- `MemoryAuthAdapter` (the in-memory dev store)
- `MemoryLimiter` (the in-memory dev limiter) or no limiter at all
...
`AuthRoot<Profile, Tenant, OrgMeta>` is parameterised by three shapes:
...
export const auth = new AuthRoot<Profile, Tenant, OrgMeta>({
  /* config */
})
```
...
- [Events →](/duck-auth/core/events) — `InMemoryEvents`, `RedisEvents`, `WebhookDeliverer`.
