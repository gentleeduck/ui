...
1. **Errors** — `AuthErrorObject.code` is stable, but the message you
   show is localised.
...
function handleAuthError(err: AuthErrorObject, req: Request) {
...
```typescript
class CustomChannel implements Channel.IChannel {
