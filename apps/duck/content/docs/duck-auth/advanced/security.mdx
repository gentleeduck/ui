...
description: Every gate AuthRoot.strict() enforces, plus the threat model and the OWASP Top 10 mapping.
section: Advanced
---
...
- `MemoryAuthAdapter` (the dev/test in-process adapter).
- `MemoryLimiter` (or no limiter at all).
- A `Session.IStore` that is not durable across process restarts.
...
| Password leak via logs | `AuthErrorObject.toJSON()` strips 13 sensitive meta keys on every serialise. |
