...
A typed sub-object on `AuthRoot` exposing a single concern (sessions,
...
A bundle of storage backends - `Identity.IStore`, `Session.IStore`,
`Credential.IStore`, optionally `Org.IStore`. duck-auth ships
`MemoryAuthAdapter`, the Redis bundle, and a SQL bridge with Drizzle
references.
...
it rejects insecure configuration (`MemoryAdapter` in production,
