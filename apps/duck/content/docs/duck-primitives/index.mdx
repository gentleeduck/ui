...
gentleduck primitives are 50-92% smaller than their Radix equivalents because shared internals (Slot, Presence, Popper, focus-scope) are deduplicated across all primitives. Real numbers: Alert Dialog 1.6 KB vs 18.6 KB, Popover 2.4 KB vs 19.6 KB, Dialog 3.1 KB vs 10.6 KB. Install 5 Radix primitives and you pay ~25 KB in duplicated internals. With gentleduck, shared internals load once.

<ComponentPreview name="chart-benchmark-primitives" description="Module breakdown and totals" />

See the full [benchmark results](/docs/packages/duck-primitives/benchmarks) for per-component comparisons and methodology.
