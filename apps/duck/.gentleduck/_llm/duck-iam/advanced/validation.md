...
|---|---|---|
| `POLICY_LIMITS.rulesPerPolicy` | 1\_000 | rules per policy |
| `POLICY_LIMITS.actionsPerRule` | 100 | actions per rule |
| `POLICY_LIMITS.resourcesPerRule` | 100 | resources per rule |
| `POLICY_LIMITS.cartesianPerRule` | 1\_000 | actions x resources expansion |
...
* Bounded quantifier with large upper bound (`a{1,1000000}`); cap is `MAX_BOUNDED_QUANTIFIER` (default 1\_000).
