...
export async function generateMetadata(props: { params: Promise<{ slug?: string[] }> }): Promise<Metadata> {
  ...
  return {
    ...base,
    alternates: {
      canonical: absoluteUrl(`/duck-state${slugPath}`),
    },
    openGraph: {
      ...base.openGraph,
      url: absoluteUrl(`/duck-state${slugPath}`),
    },
  }
}
          ...
          <div className="sticky top-16 -mt-10 flex h-[calc(100vh-3.5rem)] flex-col py-12">
            ...
            <div className="flex shrink-0 flex-col gap-1">
              <Button asChild className="justify-start" size="sm" variant="link">
                <a
                  href={`https://github.com/gentleeduck/duck-ui/blob/master/apps/duck/content/duck-state/${_params.slug?.join('/') ?? 'index'}.mdx`}
                  rel="noreferrer"
                  target="_blank">
