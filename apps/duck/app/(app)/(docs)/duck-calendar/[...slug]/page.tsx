...
import { duckCalendarSidebar } from '~/config/sidebars'
import { absoluteUrl } from '~/lib'
import { duckCalendar } from '../../../../../.gentleduck'
export const dynamic = 'force-static'
export const dynamicParams = false
export const revalidate = false

const PKG_PREFIX = 'duck-calendar'
const sidebar = duckCalendarSidebar

function getDocFromSlug(slug: string[]) {
  const permalink = `${PKG_PREFIX}/${slug.join('/')}`
  return duckCalendar.find((doc) => [permalink, `${permalink}/index`].includes(doc.permalink)) ?? null
}

export async function generateStaticParams() {
  return duckCalendar.map((doc) => {
  ...
  return (
    <div className="container flex-1 items-start md:grid md:grid-cols-[270px_minmax(0,1fr)] md:gap-6 lg:grid-cols-[300px_minmax(0,1fr)] lg:gap-10">
      <aside
        ...
        <nav aria-label="Duck Calendar sections" className="h-full overflow-y-auto overflow-x-hidden pt-10 pb-8">
