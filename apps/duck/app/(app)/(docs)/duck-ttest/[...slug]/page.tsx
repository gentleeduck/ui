...
const PKG_PREFIX = 'duck-ttest'
const sidebar = packageSidebar(duckTtest, PKG_PREFIX)

function getDocFromSlug(slug: string[]) {
  const permalink = `${PKG_PREFIX}/${slug.join('/')}`
  return duckTtest.find((doc) => [permalink, `${permalink}/index`].includes(doc.permalink)) ?? null
}

export async function generateStaticParams() {
  return duckTtest.map((doc) => {
  ...
  return (
    <div className="container flex-1 items-start md:grid md:grid-cols-[270px_minmax(0,1fr)] md:gap-6 lg:grid-cols-[300px_minmax(0,1fr)] lg:gap-10">
      <aside
        ...
        <nav aria-label="Duck TType sections" className="h-full overflow-y-auto overflow-x-hidden pt-10 pb-8">
