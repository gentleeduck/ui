  ...
  return (
    <section className={cn('border-grid', className)} {...props}>
      <div className="container-wrapper">
        <div className="container flex flex-col items-center gap-2 border-x py-[clamp(2rem,5vw,5rem)] text-center xl:gap-4">
          {children}
        </div>
      </div>
    </section>
...
function PageHeaderHeading({ className, ...props }: React.ComponentProps<'h1'>) {
  return (
    <h1
      className={cn(
        'max-w-4xl text-balance font-semibold text-2xl text-primary uppercase leading-tighter tracking-tight sm:text-3xl md:text-4xl lg:font-semibold lg:leading-[1.1] xl:text-5xl xl:tracking-tighter',
        className,
      )}
      {...props}
    />
