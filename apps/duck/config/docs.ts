...
const corePackagesSection = docsConfig.sidebarNav.find((section) => section.title === 'Core Packages')

export function getPackageDocsConfig(packageSlug: string): IDocsConfig | null {
  const packageItem = corePackagesSection?.items?.find(
    ...
    sidebarNav: [
      {
        ...
        items: packageItem.items?.length ? packageItem.items : [{ href: packageItem.href, title: packageItem.title, items: [] }],
      },
    ],
  }
}
