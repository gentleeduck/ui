...
const _corePackagesSection = docsConfig.sidebarNav.find((s) => s.title === 'Core Packages')

function rewritePackageHrefs(
  items: NavItem[],
  from: string,
  to: string,
): NavItem[] {
  return items.map((item) => ({
    ...item,
    href: item.href?.replace(from, to),
    items: item.items ? rewritePackageHrefs(item.items, from, to) : item.items,
  }))
}

export function getPackageDocsConfig(packageSlug: string): IDocsConfig | null {
  const packageItem = _corePackagesSection?.items?.find(
    (item) => item.href === `/docs/packages/${packageSlug}`,
  )
  if (!packageItem) return null

  const from = `/docs/packages/${packageSlug}`
  const to = `/${packageSlug}`

  const rewrittenItems = packageItem.items?.length
    ? rewritePackageHrefs(packageItem.items, from, to)
    : [{ href: to, title: packageItem.title, items: [] }]
