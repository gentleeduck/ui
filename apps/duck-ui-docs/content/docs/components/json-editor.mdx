...
The `json-editor` component depends on the [`alert-dialog`](/docs/components/alert-dialog), [`button`](/docs/components/button), [`popover`](/docs/components/popover), [`react-hook-form`](/docs/components/react-hook-form), and [`sheet`](/docs/components/sheet) components.

<Step>Copy and paste the following code into your project.</Step>

<ComponentSource name="json-editor" />
...
import { Form } from '@/components/ui/react-hook-form'
...
| `dir` | `'ltr' \| 'rtl'` | -- | Text direction. Controls layout of the editor chrome (gutter, buttons). JSON content is always LTR. |
...
Pass `dir="rtl"` and `lang` for locale-aware line numbers. Use the `text` prop to translate all button labels, status messages, and dialog strings.
