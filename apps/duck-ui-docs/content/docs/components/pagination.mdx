...
| --- | --- | --- | --- |
| `dir` | `'ltr' \| 'rtl'` | `'ltr'` | Layout direction for pagination and controls |
...
Use the `text` prop on `PaginationPrevious`, `PaginationNext`, and `PaginationEllipsis` to provide translated labels. Set `dir="rtl"` on `Pagination` for right-to-left layout and keep markup in logical order (`Previous`, pages, `Next`): visual mirroring is handled automatically. Chevron icons and wrapper controls adapt automatically.
