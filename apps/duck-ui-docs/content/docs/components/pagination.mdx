...
Use the `text` prop on `PaginationPrevious`, `PaginationNext`, and `PaginationEllipsis` to provide translated labels. Wrap in `dir="rtl"` for right-to-left layout. Chevron icons auto-flip via `rtl:rotate-180`.
