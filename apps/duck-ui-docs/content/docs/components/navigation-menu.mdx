...
<ComponentPreview name="navigation-menu-1" className="[&>div>div>div[duck-preview]]:overflow-visible!" description="A navigation menu with dropdown content panels and a documentation link." />
...
```bash
npm install @radix-ui/react-navigation-menu @gentleduck/primitives @gentleduck/libs @gentleduck/variants lucide-react
```
...
| `dir` | `'ltr' \| 'rtl'` | -- | Text direction. Resolved at the primitive layer via `useDirection` and inherited by all descendants through context. |
...
<ComponentPreview
  ...
  className="[&>div>div>div[duck-preview]]:overflow-visible
  [&>div>div>div[duck-preview]]:justify-end mb-40" 
