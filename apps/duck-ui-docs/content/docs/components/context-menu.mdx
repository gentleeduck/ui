...
### Animated Context Menu (Motion)

Use `MotionContextMenuContent` for animated context menus. The transform origin is fixed to top-left since the menu appears at the cursor position.
