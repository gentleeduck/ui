...
Replaces `AccordionItem`. Keeps the `<details>` element open for motion height animation. Same props. Requires the `motion` package.

### MotionAccordionTrigger

Replaces `AccordionTrigger`. Chevron rotation driven by motion instead of CSS `group-open`. Same props. Requires the `motion` package.

### MotionAccordionContent

Replaces `AccordionContent`. Animates height from 0 to auto with staggered blur and opacity fade using heightAuto preset. Requires the `motion` package.
