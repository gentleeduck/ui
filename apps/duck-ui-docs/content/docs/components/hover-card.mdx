...
Same props as `HoverCard`. Wraps with `useMotionRoot` for exit animation support. Requires the `motion` package.

### MotionHoverCardContent

Same props as `HoverCardContent`. Adds directional scale, blur, and opacity enter/exit animation. Requires the `motion` package.
