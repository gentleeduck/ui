...
The `HoverCard` component accepts a `dir` prop on the root element to control the reading direction. Set `dir="rtl"` to mirror the hover card positioning and content layout for right-to-left languages.
