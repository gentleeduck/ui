...
Same props as `BreadcrumbItem` plus `index` (number, default 0) for stagger delay. Each item fades up with blur at `index * 50ms` delay. Requires the `motion` package.
