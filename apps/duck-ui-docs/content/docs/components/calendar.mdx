...
| --- | --- | --- | --- |
| `adapter` | `DateAdapter<Date>` | `NativeAdapter` | Date adapter for alternative calendar systems (Islamic, Persian, Hebrew) |
...
| `renderDay` | `(day: CalendarDay<Date>, children: ReactNode) => ReactNode` | -- | Custom render function for day cell content |
...
| `renderFooter` | `(months: CalendarMonth<Date>[]) => ReactNode` | -- | Render content below the calendar grid |
...
Customize the content inside each day cell. Receives the `CalendarDay` object and the default children (the date number).
...
The `day` object exposes: `date`, `isToday`, `isSelected`, `isDisabled`, `isOutside`, `isRangeStart`, `isRangeEnd`, `isRangeMiddle`.
...
```tsx showLineNumbers
<Calendar
  renderHeader={({ title, goToPrevMonth, goToNextMonth, isPrevDisabled, isNextDisabled }) => (
...
| `data-slot="calendar-day"` | Always present on day cell buttons |
