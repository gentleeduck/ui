...
Use `MotionSwitch` for a bounce animation powered by [motion](https://motion.dev). The switch bounces (scale + rotate) on each toggle.

<ComponentPreview
  name="switch-4"
  description="Switch with motion-powered bounce on toggle."
/>

<Callout icon={<Zap />}>
Requires the `motion` package. Use `MotionSwitch` instead of `Switch`. Same props. The regular `Switch` is perfectly fine - this is an optional enhancement.
</Callout>
...
Bounces (scale + rotate) on toggle, same as `MotionCheckbox`. Requires the `motion` package.

| Prop | Type | Default | Description |
| --- | --- | --- | --- |
| `...props` | `SwitchProps` | - | All props from `Switch` are supported |
