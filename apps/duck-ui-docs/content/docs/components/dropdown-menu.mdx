...
### Animated Dropdown Menu (Motion)

Use `MotionDropdownMenuContent` for smooth enter/exit animations with a bouncy spring. Requires `motion` installed and a controlled `open` state.
