...
*Also accepts all standard `react-resizable-panels` `Panel` props.*
...
| Prop | Type | Default | Description |
| --- | --- | --- | --- |
| `withHandle` | `boolean` | `false` | Whether to render the visible handle UI (grip icon) |

*Also accepts all standard `React.ComponentProps<typeof ResizablePrimitive.PanelResizeHandle>` props.*
