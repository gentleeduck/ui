...
<MermaidDiagram chart={`graph TD
  ...
  InputOTP --> |"custom"| OTPContext["OTP Context\n(slot management)"]
  InputOTP --> |"custom"| InitHook["useInputOTPInit\n(ref wiring)"]
  OTPContext --> |"manages"| SlotState["Slot Active State\n& Values"]
...
| `pattern` | `RegExp` | `/^[\w\d\p{P}\p{S}]$/u` | Regex used to validate each entered character |
