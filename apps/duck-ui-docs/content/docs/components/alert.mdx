...
Same props as `Alert`. Animates on mount with fade-up (slide up 6px + blur + opacity) using the `fadeUp` preset and `contentTransition` (250ms expo-out). Requires the `motion` package.

### MotionAlertTitle

Same props as `AlertTitle`. Fades up with blur at 100ms delay after the container. Requires the `motion` package.

### MotionAlertDescription

Same props as `AlertDescription`. Fades up with blur at 180ms delay after the title. Requires the `motion` package.
