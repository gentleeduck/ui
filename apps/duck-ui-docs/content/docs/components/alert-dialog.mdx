...
### Animated Alert Dialog (Motion)

Use `MotionAlertDialogContent` for smooth enter/exit animations. Uses a stiffer spring (stiffness 400, damping 30) to convey urgency compared to the regular dialog. Requires `motion` to be installed and a controlled `open` state.
