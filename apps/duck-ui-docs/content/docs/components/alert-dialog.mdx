...
| `dir` | `'ltr' \| 'rtl'` | `--` | Reading direction for RTL support. Inherited from the underlying Dialog primitive |
...
The `AlertDialog` component accepts a `dir` prop on the root element to control the reading direction. Since `AlertDialog` extends `Dialog` internally, setting `dir="rtl"` applies the same right-to-left layout behavior to all child content, including the header, footer, and action buttons.
