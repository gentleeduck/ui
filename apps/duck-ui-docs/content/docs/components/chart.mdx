...
*Accepts all standard Recharts `Tooltip` props.*
...
*Also accepts all Recharts `Tooltip` props (`active`, `payload`, etc.) and standard `HTMLDivElement` attributes.*

### ChartLegend

Re-export of `RechartsPrimitive.Legend`. Use this as the wrapper and pass `<ChartLegendContent />` via its `content` prop.

*Accepts all standard Recharts `Legend` props.*
