...
Same props as `Button` except `asChild` is not supported. Adds `whileTap` press feedback and animated icon/text transitions via `AnimatePresence`. Requires the `motion` package.
