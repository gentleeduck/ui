...
| `size` | `'icon' \| 'xs' \| 'sm' \| 'default' \| 'lg' \| 'xl' \| '2xl' \| '3xl'` | `'default'` | Size of the button |
| `border` | `'default' \| 'primary' \| 'secondary' \| 'destructive' \| 'warning'` | -- | Border style variant |
| `asChild` | `boolean` | -- | If `true`, renders using a custom child element via `Slot` (e.g. a `Link`) |
| `loading` | `boolean` | -- | If `true`, shows a spinning loader icon and disables the button |
| `disabled` | `boolean` | -- | Disables the button manually |
| `icon` | `React.ReactNode` | -- | Icon placed before the content (left-aligned or centered if collapsed) |
| `secondIcon` | `React.ReactNode` | -- | Icon placed after the content (right-aligned) |
...
| Prop | Type | Default | Description |
| --- | --- | --- | --- |
| `animationIcon` | `{ icon?: React.ReactNode; iconPlacement?: 'left' \| 'right' }` | -- | Animation icon configuration |
