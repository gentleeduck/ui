...
| `dir` | `'ltr' \| 'rtl'` | -- | Reading direction for RTL support |
...
The `Popover` component accepts a `dir` prop on the root element to control the reading direction. Set `dir="rtl"` to mirror the popover positioning and content layout for right-to-left languages. When `dir="rtl"` is set, `side="left"` and `side="right"` are logically swapped so the popover opens on the correct side relative to the reading direction.
