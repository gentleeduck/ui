...
Same props as `Popover`. Wraps with `useMotionRoot` for exit animation support. Requires the `motion` package.

### MotionPopoverContent

Same props as `PopoverContent`. Adds scale, blur, and opacity enter/exit animation with springBouncy transition. Requires the `motion` package.
