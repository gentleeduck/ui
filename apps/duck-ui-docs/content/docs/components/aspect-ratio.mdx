...
Same props as `AspectRatio`. Renders a `div` (not `Slot`) that scales from 0.95 with 8px blur fade on mount using `contentTransition` (250ms expo-out). Requires the `motion` package.
