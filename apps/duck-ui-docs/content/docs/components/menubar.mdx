...
Use `MotionMenubarContent` for spring-animated dropdown and `MotionMenubarItem` for staggered fade+blur item entrance powered by [motion](https://motion.dev).

<ComponentPreview name="menubar-3" description="A menubar with motion-powered animations." />

<Callout icon={<Zap />}>
Requires the `motion` package. Use `MotionMenubarContent` instead of `MenubarContent` and `MotionMenubarItem` instead of `MenubarItem`. All other sub-components stay the same.
</Callout>
...
Same props as `MenubarContent`. Adds spring scaleIn+blur entrance animation. Requires the `motion` package.

### MotionMenubarItem

Same props as `MenubarItem` plus an optional `index` prop for stagger delay (30ms per index). Adds fade+blur entrance. Requires the `motion` package.
