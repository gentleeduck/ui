...
Same props as `CheckboxWithLabel` plus an optional `index` prop for stagger delay (50ms per index). Requires the `motion` package.

### MotionCheckboxGroup

Same props as `CheckboxGroup`. Wraps the group with fadeUp and renders each item as `MotionCheckboxWithLabel` with automatic stagger. Requires the `motion` package.
