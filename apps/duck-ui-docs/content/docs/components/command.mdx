...
* **RTL Support:** Full right-to-left layout via the `dir` prop.
...
| --- | --- | --- | --- |
| `dir` | `'ltr' \| 'rtl'` | -- | Text direction. Inherited by all child components. |
...
| `dir` | `'ltr' \| 'rtl'` | Text direction for the command tree. |
...
RTL is handled at the primitive layer. Pass `dir="rtl"` on the root `Command` and every sub-component receives the direction through context -- no per-child `dir` props needed.
