...
Same props as `Combobox`. Uses `MotionButton` for the trigger and `MotionPopoverContent` for the dropdown with spring enter/exit animations. Requires the `motion` package.

### MotionComboboxItem

Same props as `ComboboxItem`. Uses `MotionCheckbox` with bounce animation on toggle. Requires the `motion` package.
