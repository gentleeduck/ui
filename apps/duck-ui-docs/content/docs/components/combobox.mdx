...
<MermaidDiagram chart={`graph TD
  ...
  CommandComp --> |"provides"| Search["Fuzzy Search"]
...
| `popoverContent` | `React.ComponentPropsWithoutRef<typeof PopoverContent>` | -- | Props spread onto `PopoverContent`. `className` is merged with the default `'w-[200px] p-0'`. |
...
*Also accepts all `CommandItem` props (except `onSelect`, which is replaced by the typed version above).*
...
*Accepts all `CommandGroup` props (`heading`, `className`, etc.).*
