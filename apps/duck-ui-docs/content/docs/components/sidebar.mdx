...
| `dir`         | `ltr` or `rtl`                    | Direction for RTL layout support.      |
...
<ComponentPreview name="sidebar-2" description="A sidebar with the default sidebar variant." />
...
The sidebar supports RTL layouts via the `dir` prop on the `Sidebar` component. The trigger icon and rail positioning adjust automatically.
