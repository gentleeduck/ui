...
### Animated Dialog (Motion)

Use `MotionDialogContent` for smooth enter/exit animations powered by the [motion](https://motion.dev) library. The dialog scales from 0.95 to 1 while fading in, and reverses on exit. Requires `motion` to be installed and a controlled `open` state.
...
`MotionDialogContent` requires the `motion` package to be installed and a controlled `open` prop. Wrap your app in `MotionProvider` from `@gentleduck/motion/motion-provider` for consistent defaults.
</Callout>
