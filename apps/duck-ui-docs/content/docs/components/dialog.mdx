...
| Prop | Type | Default | Description |
| --- | --- | --- | --- |
| `asChild` | `boolean` | `false` | Renders the child as trigger element instead of a default button |
...
The `Dialog` component accepts a `dir` prop on the root element to control the reading direction. Set `dir="rtl"` to render the dialog content in right-to-left mode. This affects the layout direction of the dialog header, footer, and close button positioning.
