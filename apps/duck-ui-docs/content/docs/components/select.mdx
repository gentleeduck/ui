...
Use `MotionSelect` and `MotionSelectContent` for smooth enter/exit animations powered by [motion](https://motion.dev). The dropdown scales and fades with blur using a bouncy spring.
...
Same props as `Select`. Wraps with `useMotionRoot` for exit animation support. Requires the `motion` package.

### MotionSelectContent

Same props as `SelectContent`. Adds scale, blur, and opacity enter/exit animation with springBouncy transition. Requires the `motion` package.
