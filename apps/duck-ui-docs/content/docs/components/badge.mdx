...
Same props as `Badge` except `asChild` is not supported. Fades up with blur on mount using the `fadeUp` preset and `contentTransition` (250ms expo-out). Requires the `motion` package.
