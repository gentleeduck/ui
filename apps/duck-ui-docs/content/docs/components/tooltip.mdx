...
Same props as `Tooltip`. Wraps with `useMotionRoot` for exit animation support. Requires the `motion` package.

### MotionTooltipContent

Same props as `TooltipContent`. Adds directional scale, blur, and opacity enter/exit animation with ultra-fast tweenMicro (100ms) transition. Requires the `motion` package.
