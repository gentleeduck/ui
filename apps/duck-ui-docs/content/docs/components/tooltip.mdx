...
The `Tooltip` component accepts a `dir` prop on the root element to control the reading direction. Set `dir="rtl"` to mirror the tooltip positioning for right-to-left languages.
