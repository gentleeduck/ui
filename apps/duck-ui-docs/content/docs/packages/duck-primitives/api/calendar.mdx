---
title: "Calendar"
description: "Compound components for building accessible calendars, powered by @gentleduck/calendar."
---
...
| :--- | :--- | :--- | :--- |
| `adapter` | `DateAdapter<Date>` | **required** | Date adapter instance |
...
| Prop | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| `formatMonth` | `(month: Date, adapter: DateAdapter<Date>) => string` | -- | Custom formatter for the title. Defaults to `"March 2026"` format |
...
The day grid container. Renders a `div` with `role="grid"` and `aria-roledescription="calendar"`.
...
Renders the weekday header row. Each weekday renders as an `abbr` with `role="columnheader"`.
...
| Prop | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| `day` | `CalendarDay<Date>` | **required** | Day data from the calendar grid |
...
- `Calendar.Grid` renders `role="grid"` with `aria-roledescription="calendar"`
