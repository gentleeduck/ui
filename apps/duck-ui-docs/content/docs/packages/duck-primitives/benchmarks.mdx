...
**Radio Group** is one case where Radix is smaller (1.0 KB vs 1.9 KB). The extra size comes from built-in roving focus, arrow-key navigation, and collection management that gentleduck bakes into the primitive. Radix Radio Group delegates some of this to separate internal packages, which appear smaller per-component but add up across your full install. We show honest numbers  -  not every primitive is smaller in isolation.
...
- **Maturity**  -  battle-tested in production at thousands of companies
- **Ecosystem**  -  shadcn/ui, Vercel, and many design systems are built on Radix
- **React 18 support**  -  gentleduck requires React 19
