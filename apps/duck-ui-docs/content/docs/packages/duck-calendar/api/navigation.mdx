...
goToMonth(adapter: DateAdapter<TDate>, current: TDate, month: number) => TDate
```
...
```tsx
goToYear(adapter: DateAdapter<TDate>, current: TDate, year: number) => TDate
```
...
```tsx
goToNextMonth(adapter: DateAdapter<TDate>, current: TDate) => TDate
```
...
```tsx
goToPrevMonth(adapter: DateAdapter<TDate>, current: TDate) => TDate
```
