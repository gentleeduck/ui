...
`useCalendar` accepts a `UseCalendarConfig<TDate, M>` object:

| Prop | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| `adapter` | `DateAdapter<TDate>` | **required** | Date adapter instance |
| `mode` | `SelectionMode` | **required** | `'single'`, `'range'`, or `'multi'` |
...
| `numberingSystem` | `string` | Intl numbering system (e.g. 'arab', 'arabext') |
| `calendar` | `string` | Intl calendar system (e.g. 'islamic', 'persian', 'hebrew') |
...
| `value` | `CalendarValue<TDate, M>` | Current selection. Shape depends on mode |
| `focusedDate` | `TDate` | Date with keyboard focus |
| `viewMode` | `ViewMode` | `'days'`, `'months'`, or `'years'` |
| `weeks` | `CalendarWeek<TDate>[]` | Decorated weeks for the first month |
| `months` | `CalendarMonth<TDate>[]` | All month grids (for multi-month) |
...
| `setViewMode` | `(mode: ViewMode) => void` | Switch between day/month/year view |
...
| :--- | :--- | :--- |
| `getDayProps` | `(day: CalendarDay<TDate>) => DayProps` | Spread onto each day cell |
