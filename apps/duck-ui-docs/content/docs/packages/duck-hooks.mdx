...
Hooks are the glue between React's render cycle and the real world -- browser APIs, external state, user interactions. Rather than scattering custom hooks across components, duck-hooks centralizes them as tested, typed utilities. Each hook solves one problem (media queries, local storage, intersection observer) and composes with others.
