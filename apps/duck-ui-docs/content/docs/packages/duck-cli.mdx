...
Most component CLIs can add components. gentleduck/cli can also update them with an interactive merge, remove them cleanly, and diff them against the registry with syntax highlighting.

The standout feature is the **update command**. When you update a component that you have locally modified, the CLI launches an interactive merge screen in your terminal. You see each change as a hunk and choose per-hunk what to keep:
...
After switching, you also get `update`, `remove`, and `list` which shadcn does not provide. The `update` command with interactive merge is particularly useful for teams that customize components heavily - you can pull upstream improvements without losing your local changes.
