...
This page is the full config reference. The important framing is: for new non-UI consumers, start with `collections`. The older UI-shaped fields still work, but they are now a compatibility surface rather than the preferred starting point.
...
If you are building a UI/docs registry, you can still use `sources`, `registries`, `targetPaths`, `importMappings`, `componentIndex`, `colors`, `themes`, and related fields. They remain supported through the compatibility layer.
...
- you are building a package index or repository catalog
- you want extensions to read arbitrary data without pretending it is a UI component registry
- you want a small, generic config with minimal domain assumptions
...
`pipeline` controls the current core phases:

- `index`
- `components`

For generic collection-driven builds, it is valid to disable both and let extensions own every emitted artifact.
...
Extensions are where custom behavior lives.
...
- keep static data in config
- let extensions read collections and existing artifacts
- let extensions register outputs explicitly
...
- collection data files resolve relative to the file that declared them
- collection source paths resolve relative to the file that declared them
- source paths and external data files from compatibility fields follow the same rule
