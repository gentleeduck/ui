...
description: "Step-by-step course for building an Arch-style package repository index with registry-build."
---
...
- `pipeline.index` and `pipeline.components` are disabled because this build does not need the UI-oriented core outputs
