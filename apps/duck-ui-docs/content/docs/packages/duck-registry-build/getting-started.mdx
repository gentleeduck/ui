...
If you enable the core UI-oriented phases or extensions, you can still get outputs such as `public/r/index.json`, per-item JSON files, or `__ui_registry__/index.tsx`.
...
<Step>The loader resolved collections and compatibility fields</Step>
...
<Step>The runner created the shared build context</Step>

The context now carries resolved collections, output paths, cache helpers, output registration, and extension APIs.

<Step>The core phases ran only if you left them enabled</Step>

In this example they are disabled because the custom extension owns the outputs. In a UI registry build, the core can still generate `index.json` and per-item JSON payloads.
