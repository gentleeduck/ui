...
<MermaidDiagram chart={`flowchart TD
    ...
    F --> G[Core Phases]
    F --> H[Extensions]
    H --> I[Arch Repo Output]
    H --> J[Component Index]
    H --> K[Colors and Themes]
    G --> L[Generated Outputs]
    H --> L
    F --> M[.registry-build Cache]
`} />
...
| Core phases | Build base index and JSON artifacts when enabled | Stable machine-readable artifacts |
| Extensions | Add validation, banners, repository indexes, component indexes, colors/themes, or custom outputs | No hidden default plugins |
