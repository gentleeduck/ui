...
Variant management is the bridge between design tokens and component props. duck-variants provides a type-safe API for declaring visual variants (size, color, state) that maps directly to CSS classes. It replaces ad-hoc conditional class logic with a schema that's readable, extensible, and catches invalid combinations at compile time.
