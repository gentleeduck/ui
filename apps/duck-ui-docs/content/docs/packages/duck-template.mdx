...
Project scaffolding should encode best practices, not just file structure. duck-template generates projects pre-configured with the patterns we've found work best — TypeScript strict mode, path aliases, tailwind configuration, and component conventions — so you start productive instead of spending a day on setup.
