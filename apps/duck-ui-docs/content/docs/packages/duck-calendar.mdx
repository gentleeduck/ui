...
[data-range-middle="true"] { @apply bg-accent; }
...
| --- | --- | --- |
| Core (adapter + grid + selection + nav + time) | **2.5 KB** | No React, pure functions |
| React hooks (useCalendar + useTimePicker + useDateTime) | **4.2 KB** | Requires React peer dep |
| Full package | **7.0 KB** | Everything exported |

### vs react-day-picker v9

| Metric | duck-calendar | react-day-picker | Improvement |
| --- | --- | --- | --- |
| Full gzipped | 7.0 KB | ~20 KB | **65% smaller** |
| CSS shipped | 0 KB | ~3 KB | Zero CSS (data attributes) |
| Dependencies | 0 | date-fns (optional) | Zero deps |
| Tree-shakeable | Yes (`sideEffects: false`) | Partial | Full tree-shaking |

### Rendering Performance

Core function benchmarks (measured over 1000 iterations):
