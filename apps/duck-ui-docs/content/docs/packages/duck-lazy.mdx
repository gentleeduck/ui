...
Lazy loading should be invisible to the user and effortless for the developer. duck-lazy wraps React's `lazy()` and `Suspense` with sensible defaults -- loading skeletons, error boundaries, and intersection observer support -- so components and images load only when needed, without boilerplate.
...
| `elementRef` | `React.Ref` | Ref to attach to the observed element |
