...
## Motion Design Tokens <Badge>feat</Badge>

Created `@gentleduck/motion` design tokens for consistent animation durations and easing.

- Centralized `prefers-reduced-motion` styles via shared helpers
- Refactored `block-wrapper` and other components to use `duck-motion` utilities
- Added `AnimPopoverVariants` and removed orphaned animation variants

See the [motion package docs](/docs/packages/duck-primitives/guides/animation) for more.
