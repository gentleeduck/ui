      ...
      <AnimatePresence>
        {isLiftMode && (
          <motion.div
            ...
            exit={{
              opacity: 0,
              transition: motionTransition(prefersReducedMotion, { duration: 0.38, ease: 'easeOut' }),
            }}
            initial={{ opacity: prefersReducedMotion ? 1 : 0 }}
            transition={motionTransition(prefersReducedMotion, { delay: 0.18, duration: 0.2, ease: 'easeOut' })}
          />
        )}
      </AnimatePresence>
    </>
