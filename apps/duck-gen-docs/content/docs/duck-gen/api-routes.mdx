...
**Important:** Always add explicit return type annotations to your controller methods.
If the return type is inferred as `any`, Duck Gen will:
- Print a warning during generation.
- Convert `any` to `unknown` if `normalizeAnyToUnknown` is enabled.
