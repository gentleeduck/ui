...
```text
1. Write NestJS controllers with typed DTOs and @duckgen message tags
                          ↓
2. Run `pnpm exec duck-gen` to generate ApiRoutes and message types
                          ↓
3. Create Duck Query client with `createDuckQueryClient<ApiRoutes>()`
                          ↓
4. Make type-safe API calls — paths, bodies, and responses are enforced
                          ↓
5. Build type-safe i18n using DuckgenScopedI18nByGroup
                          ↓
6. When the server changes, re-run duck-gen — TypeScript catches mismatches
```
